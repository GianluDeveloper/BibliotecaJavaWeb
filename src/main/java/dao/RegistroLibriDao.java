...
public class RegistroLibriDao {
	...
	public boolean insert(Registro d) {
		Object[] campiString = { d.getIdLibro(), d.getIdCliente(), d.getIdDipendente(), d.getData_prestito().split("/"),
				d.getData_scadenza().split("/"), d.getData_rientro().split("/") };
	...
	public boolean update(Registro d) {
		Object[] campiString = { d.getIdLibro(), d.getIdCliente(), d.getIdDipendente(), d.getData_prestito().split("/"),
				d.getData_scadenza().split("/"), d.getData_rientro().split("/"), d.getId() };
		ObjSql connettore = new ObjSql();
	...
	public Registro findById(int id) {
		...
		String sql = "SELECT `id`,`idLibro`, `idCliente`,`idDipendente`,`data_prestito`,"
				+ "`data_scadenza`,`data_rientro`" + "FROM `registro` WHERE `id` = ?";
		boolean resp = connettore.sql(sql, campiString);
	...
	public List<Registro> findBykv(String k, String v) {
		...
		String[] allowed = { "id", "idLibro", "idCliente", "idDipendente", "data_prestito", "data_scadenza",
				"data_rientro" };
		...
		ObjSql connettore = new ObjSql();
		String sql = "SELECT `id`,`idLibro`, `idCliente`,`idDipendente`,`data_prestito`,"
				+ "`data_scadenza`,`data_rientro` " + "FROM `registro` WHERE `" + k + "`=?";
		boolean resp = connettore.sql(sql, campiString);
	...
	public List<Registro> findAll() {
		...
		ObjSql connettore = new ObjSql();
		String sql = "SELECT `id`,`idLibro`, `idCliente`,`idDipendente`,`data_prestito`,"
				+ "`data_scadenza`,`data_rientro`" + "FROM `registro` WHERE ?";
		boolean resp = connettore.sql(sql, campiString);
