...
public class DipendentiDao {
	...
	public boolean update(Dipendenti d) {
		Object[] campiString = { d.getNome(), d.getCognome(), d.getTelefono(), d.isAdmin(), d.getMatricola() };
		ObjSql connettore = new ObjSql();
	...
	public Dipendenti findById(int id) {
		...
		ObjSql connettore = new ObjSql();
		String sql = "SELECT `id`, `nome`, `cognome`, `telefono`,`admin` FROM `dipendente` " + "WHERE `id`= ?";
		boolean resp = connettore.sql(sql, campiString);
	...
	public List<Dipendenti> findBykv(String k, String v) {
		...
		ObjSql connettore = new ObjSql();
		String sql = "SELECT `id`, `nome`, `cognome`, `telefono`,`admin` FROM `dipendente` " + "WHERE `" + k + "`=?";
	...
	public List<Dipendenti> findAll() {
		...
		ObjSql connettore = new ObjSql();
		String sql = "SELECT `id`, `nome`, `cognome`, `telefono`,`admin` FROM `dipendente` " + "WHERE ?";
		boolean resp = connettore.sql(sql, campiString);
